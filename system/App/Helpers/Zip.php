<?php
	...
	|--------------------------------------------------------------------------------------
	| @package      AVE.cms
	| @file         system/App/Helpers/Zip.php
	...
	class Zip
	{
		...
		public function addDir($directory)
		{
			foreach ((array) $directory as $dir) {
				$dir = $this->_safe_entry_name($dir, true);
		...
		{
			$filepath = $this->_safe_entry_name($filepath, false);
		...
		public function extract($zip_file, $target_dir = null)
		{
			...
			if ($target_dir === '' || (!is_dir($target_dir) && !@mkdir($target_dir, 0755, true))) {
				return false;
			}

			$target_root = realpath($target_dir);
			if ($target_root === false || !is_dir($target_root)) {
				return false;
			}

			$target_root = rtrim(str_replace('\\', '/', $target_root), '/');
			...
			foreach ($files as $file => $trash) {
				try {
					$safe_file = $this->_safe_entry_name($file, substr($file, -1) === '/');
				...
				$is_directory = substr($safe_file, -1) === '/';
				$relative = rtrim($safe_file, '/');
				$location = $target_root . '/' . $relative;
				$directory = $is_directory ? $location : dirname($location);
				if (!$this->_ensure_safe_directory($directory, $target_root)) {
					$this->_close_archive();
					$this->_archive_info = array();
					return false;
				}
				...
				if (is_link($location) || is_dir($location) || $this->_extract_file($file, $location) === false) {
					@unlink($location);
					$this->_close_archive();
					$this->_archive_info = array();
					return false;
		...
		private function _safe_entry_name($name, $directory)
		{
			$name = str_replace('\\', '/', trim((string) $name));
			$is_directory = (bool) $directory || substr($name, -1) === '/';
			$name = rtrim($name, '/');
			if ($name === '' || strlen($name) > 1024 || strpos($name, "\0") !== false
				|| preg_match('/[\x00-\x1F\x7F]/', $name) || $name[0] === '/'
				|| preg_match('/^[A-Za-z]:\//', $name)) {
				throw new \InvalidArgumentException('ZIP entry contains an unsafe path');
			}
		...
		private function _ensure_safe_directory($directory, $target_root)
		{
			$relative = ltrim(substr(str_replace('\\', '/', $directory), strlen($target_root)), '/');
			$current = $target_root;
			foreach ($relative === '' ? array() : explode('/', $relative) as $segment) {
				...
				if ($real === false || !$this->_inside_target($real, $target_root)) {
					return false;
				}
			}
		...
		private function _inside_target($path, $target_root)
		{
			$path = rtrim(str_replace('\\', '/', (string) $path), '/');
			return $path === $target_root || strpos($path, $target_root . '/') === 0;
		}

		private function _close_archive()
		{
			if (is_resource($this->farc)) {
				fclose($this->farc);
			}
