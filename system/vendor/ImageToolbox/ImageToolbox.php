	...
	class Image_Toolbox
	{
		...
		function _loadFile($filename)
		{
			if (file_exists($filename))
			{
				...
				switch ($filedata['type'])
				{
					...
					case 18:
						$filedata['resource'] = imagecreatefromwebp($filename);
						break;
		...
		function output($output_type = false, $output_quality = false, $dither = false)
		{
			...
			switch ($output_type)
			{
				...
				case 'webp':
				case 'WEBP':
					if ($this->_types[18]['supported'] < 2)
					{
						trigger_error($this->_error_prefix . 'Imagetype ('.$this->_types[18]['ext'].') not supported for creating/writing.', E_USER_ERROR);
						return null;
					}

					header ('Content-type: ' . $this->_types[18]['mime']);
		...
		function save ($filename, $output_type = false, $output_quality = false, $dither = false)
		{
			...
			switch ($output_type)
			{
				...
				case 'WEBP':
					...
					imagewebp($this->_img['main']['resource'], $filename, $output_quality);
					break;
		...
		function newOutputSize($width, $height, $mode = 0, $autorotate = false, $bgcolor = '#000000')
		{
			...
			if ($this->_img['main']['type'] == 3)
			{
				...
				imagefilledrectangle($dummy, 0, 0, $this->_img['main']['width'], $this->_img['main']['height'], $transparent);
			}
			...
			// PNG 8
			if ($this->_img['main']['type'] == 4)
			{
				//Debug::_echo($this->_img, true);
			}
