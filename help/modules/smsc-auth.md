...
Имя отправителя должно состоять из латинских букв и цифр и занимать не более
11 символов либо быть цифровым номером длиной до 15 цифр. Сначала его нужно
зарегистрировать и дождаться допуска в кабинете SMSC. Если поле оставить пустым,
SMSC использует системное имя.
...
| Хранить историю | Сколько дней сохранять журнал отправок и попыток входа. |
...
## Баланс и история входов

На вкладке `Настройки` показываются текущий баланс SMSC, состояние подключения
и статус выбранного имени отправителя. Кнопка `Обновить данные SMSC` запрашивает
свежие сведения у сервиса; между проверками результат кешируется на одну минуту.

На отдельной вкладке `История входов` собран журнал. В него попадают отправка и
ошибка отправки SMS, ограничения частоты, неверный,
просроченный или уже использованный код, успешный вход и создание аккаунта. Для
разбора проблем доступны дата, маска и хеш телефона, IP, браузер, пользователь и
ID сообщения SMSC. Сам одноразовый код и полный номер телефона в журнал никогда
не записываются.

Фильтры позволяют найти события по маске телефона, IP или ID SMSC и оставить
только нужное событие или результат. Старые строки удаляются автоматически по
сроку из настройки `Хранить историю`.
...
При удалении стираются настройки SMSC, история незавершённых кодов и журнал входов. Пользователи,
