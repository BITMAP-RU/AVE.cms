...
| Событие | Назначение |
| --- | --- |
| `http.outbound.requesting` | Общий клиент подготовил исходящий запрос. |
| `http.outbound.responded` | Удалённый сервис успешно ответил. |
| `http.outbound.failed` | Соединение, проверка адреса или ответ завершились ошибкой. |

В событиях доступны метод, схема и хост без пути и параметров, размер, HTTP-статус,
длительность и код источника. Тела запросов и ответов, заголовки, ключи,
подписи и персональные данные не передаются.
